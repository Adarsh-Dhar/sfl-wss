...
function createApiRouter(binanceClient) {
    ...
    router.get('/token-data/:token', function (req, res) {
        try {
            ...
            const connections = binanceClient['connections'];
            ...
            const priceLog = binanceClient['priceLog'];
            const allLogs = priceLog.filter(entry => entry.prices && entry.prices[token.toLowerCase()] !== undefined);
            res.json({
                status: 'success',
                token,
                isSubscribed,
